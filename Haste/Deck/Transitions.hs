...
data Transition = Transition {
    ...
    transitionStep     :: Progress  -- ^ Progress of the animation as a floating
                       ...
                       -> NextSlide -- ^ Are we going forward or backward?
    ...
    transitionSetup    :: NextSlide -> Elem -> Elem -> Elem -> IO (),
    ...
    --   Same arguments as 'transitionStep'.
    transitionFinished :: NextSlide -> Elem -> Elem -> Elem -> IO ()
  }
...
none = Transition {
    ...
    transitionStep     = \_ _ _ _ _ -> return (),
    transitionSetup    = \_ _ _ _ -> return (),
    transitionFinished = \_ _ _ _ -> return ()
  }
...
pan = none {
    ...
    , transitionSetup = \dir parent old new -> do
        if dir == Prev
          then insertChildBefore parent old new
          else appendChild parent new

    , transitionStep = \progress dir _ old new -> do
        let oldpct
              | dir == Prev = show (progress * 100) ++ "%"
              | otherwise   = show (0 - progress * 100) ++ "%"
            newpct
              | dir == Prev = show (progress * 100 - 100) ++ "%"
...
fade = none {
    ...
    , transitionSetup = \_ parent _ new -> do
    ...
    , transitionStep = \progress _ _ old new -> do
        if progress < 0.5
...
fadeOver :: Transition
fadeOver = fade {
    transitionStep = \progress _ _ _ new -> do
       set new [style "opacity" =: show progress]
  }
